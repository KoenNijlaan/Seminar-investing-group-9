...
(EW) and value-weighted (VW) average weekly returns and Fama--French three-factor (FF3)
alphas are reported. Table~\ref{tab:portfolio_spreads} presents the results.

\input{../data_final/results/tables/tab_portfolio_spreads.tex}
...
\input{../data_final/results/tables/tab_fm_aggregate.tex}
...
\input{../data_final/results/tables/tab_fm_decomposition.tex}
...
\input{../data_final/results/tables/tab_wald_tests.tex}
...
\input{../data_final/results/tables/tab_crisis_state.tex}
...
\input{../data_final/results/tables/tab_illiquidity.tex}
...
comparable to the total RSJ spread, while systematic RSJ is near zero. All RES
long-short means are negligible.}
...
rebalancing) over January 1993 to December 2024 for RSJ total (L$-$H), RSJ
idiosyncratic M2 (L$-$H), RES total (H$-$L), and RES idiosyncratic (H$-$L). The RSJ
total and idiosyncratic strategies accumulate returns persistently throughout the sample;
RES long-short portfolios remain flat, consistent with the insignificant mean spreads
in Table~\ref{tab:portfolio_spreads}.}
...
\input{../data_final/results/tables/tab_fm_supplementary.tex}
